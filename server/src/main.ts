...
import { Prisma, Role, CriterionType, ApplicationStatus, FieldType, TeacherCategory } from '@prisma/client';
...
import { assertApplicationItems, assertCriterionPayload, assertOlympiadEntry, assertRegisterPayload, assertWritableEvidence, assertTeacherRole, assertPeriodPayload, assertTeacherCategory } from './validation.js';
...
const publicUser = (u: any) => ({ id: u.id, fullName: u.fullName, email: u.email, role: u.role, position: u.position, teacherCategory: u.teacherCategory, school: u.school });
...
export class ApiController {
  ...
  async register(@Body() b: any) {
    ...
    if (b.role === Role.DEPUTY && await this.db.user.findFirst({ where: { schoolId: school.id, role: Role.DEPUTY } })) throw new BadRequestException({ code: 'DEPUTY_EXISTS', message: 'В школе уже зарегистрирован заместитель директора' });
    ...
    const user = await this.db.user.create({ data: { fullName: b.fullName.trim(), email: b.email.trim().toLowerCase(), passwordHash: await bcrypt.hash(b.password, 12), role: b.role, position: b.position || null, teacherCategory: b.teacherCategory || null, schoolId: school.id }, include: { school: true } });
  ...
  async updateProfile(@Req() req: any, @Body() b: any) { const data: any = {}; if (b.fullName !== undefined) data.fullName = String(b.fullName).trim(); if (b.position !== undefined) data.position = String(b.position).trim() || null; if (b.teacherCategory !== undefined) { assertTeacherCategory(b.teacherCategory); data.teacherCategory = b.teacherCategory || null; } if (!data.fullName && data.fullName !== undefined) throw new BadRequestException({ code: 'VALIDATION', message: 'Укажите ФИО' }); const user = await this.db.user.update({ where: { id: req.user.userId }, data, include: { school: true } }); await this.audit(req.user.userId, req.user.schoolId, 'PROFILE_UPDATE', 'User', user.id); return publicUser(user); }
  ...
  async createPeriod(@Req() req: any, @Body() b: any) {
    ...
    const created = await this.db.$transaction(async (tx) => {
      ...
      return tx.period.create({ data: { label: String(b.label).trim(), startsAt: new Date(b.startsAt), endsAt: new Date(b.endsAt), active: true, fundAmount: this.fundAmountOf(b.fundAmount) } });
    });
  ...
  async updatePeriod(@Req() req: any, @Param('id') id: string, @Body() b: any) {
    ...
    // Частичное обновление: можно изменить только фонд, только название или даты.
    const data: any = {};
    ...
    if (b.fundAmount !== undefined) data.fundAmount = this.fundAmountOf(b.fundAmount);
    if (!Object.keys(data).length) throw new BadRequestException({ code: 'PERIOD_INVALID', message: 'Укажите данные для обновления периода' });
    assertPeriodPayload({ label: data.label ?? period.label, startsAt: data.startsAt ?? period.startsAt, endsAt: data.endsAt ?? period.endsAt, fundAmount: data.fundAmount ?? period.fundAmount });
    const updated = await this.db.period.update({ where: { id }, data });
  ...
  async listUsers(@Req() req: any) { this.onlyDeputy(req.user); return this.db.user.findMany({ where: { schoolId: req.user.schoolId, role: Role.TEACHER }, select: { id: true, fullName: true, email: true, position: true, teacherCategory: true, school: true }, orderBy: { fullName: 'asc' } }); }
  ...
  async patchApplication(@Req() req: any, @Param('id') id: string, @Body() b: any) {
    ...
    const teacher = await this.db.user.findUnique({ where: { id: a.teacherId }, select: { teacherCategory: true } });
    const items = incoming.map((i: any) => this.calculateItem(i, criteria.find((c) => c.id === i.criterionId), teacher?.teacherCategory));
  ...
  // Очистка очереди: заместитель директора удаляет отклонённые и ошибочные
  // заявки, учитель — свой черновик или возвращённую на доработку заявку.
  // Утверждённые заявки остаются в реестре выплат и не подлежат удалению.
  @UseGuards(AuthGuard) @Delete('applications/:id')
  async deleteApplication(@Req() req: any, @Param('id') id: string) {
    const a = await this.db.application.findFirst({ where: { id, schoolId: req.user.schoolId, ...(req.user.role === Role.TEACHER ? { teacherId: req.user.userId } : {}) }, include: { items: { select: { id: true } }, files: { select: { id: true, path: true } } } });
    if (!a) throw new BadRequestException({ code: 'NOT_FOUND', message: 'Заявка не найдена' });
    if (a.status !== ApplicationStatus.DRAFT && a.status !== ApplicationStatus.REJECTED) throw new BadRequestException({ code: 'STATUS_LOCKED', message: 'Можно удалять только черновики и отклонённые заявки' });
    await this.db.application.delete({ where: { id: a.id } });
    //Cascade cleans DB rows; remove the stored evidence files from disk as well.
    for (const file of a.files) { try { await unlink(path.join(storageDir(), file.path)); } catch { /* file may already be gone */ } }
    await this.audit(req.user.userId, req.user.schoolId, 'APPLICATION_DELETE', 'Application', a.id, { status: a.status });
    return { id: a.id };
  }
  ...
  async export(@Req() req: any, @Param('periodId') periodId: string, @Res() res: any) { this.onlyDeputy(req.user); const [apps, period] = await Promise.all([ this.db.application.findMany({ where: { periodId, schoolId: req.user.schoolId, status: ApplicationStatus.APPROVED }, include: { teacher: { select: { fullName: true } }, items: true } }), this.db.period.findUnique({ where: { id: periodId }, select: { fundAmount: true } }) ]); const potential = apps.reduce((sum, a) => sum + a.total, 0); const fund = period?.fundAmount ?? 0; // Коэффициент K: 1, если потенциальная сумма укладывается в фонд, иначе Фонд / Потенциальная сумма.
    const coefficient = fund > 0 && potential > fund ? fund / potential : 1; const wb = new ExcelJS.Workbook(); const ws = wb.addWorksheet('Заявки'); ws.columns = [{ header: 'Учитель', key: 'teacher', width: 28 }, { header: 'Статус', key: 'status', width: 14 }, { header: 'Потенциальная сумма, ₽', key: 'total', width: 16 }, { header: 'Выплата с учётом коэффициента, ₽', key: 'payout', width: 20 }]; apps.forEach((a) => ws.addRow({ teacher: a.teacher.fullName, status: 'Утверждена', total: a.total, payout: Math.round(a.total * coefficient) })); const cs = wb.addWorksheet('Критерии'); cs.columns = [{ header: 'Критерий', key: 'title', width: 45 }, { header: 'Выплачено, ₽', key: 'amount', width: 16 }, { header: 'Выплат', key: 'count', width: 10 }]; const items = apps.flatMap((a) => a.items); const byCriterion = new Map<string, { title: string; amount: number; count: number }>(); items.forEach((item) => { const row = byCriterion.get(item.criterionId) || { title: item.titleSnapshot, amount: 0, count: 0 }; row.amount += item.amount; row.count += 1; byCriterion.set(item.criterionId, row); }); for (const row of byCriterion.values()) cs.addRow({ title: row.title, amount: row.amount, count: row.count }); const totals = wb.addWorksheet('Итоги'); totals.columns = [{ header: 'Показатель', key: 'metric', width: 32 }, { header: 'Значение', key: 'value', width: 18 }]; totals.addRow({ metric: 'Утверждённых заявок', value: apps.length }); totals.addRow({ metric: 'Фонд стимулирующих выплат, ₽', value: fund || 'не задан' }); totals.addRow({ metric: 'Потенциальная сумма выплат, ₽', value: potential }); totals.addRow({ metric: 'Коэффициент K', value: Math.round(coefficient * 10000) / 10000 }); totals.addRow({ metric: 'Итого к выплате, ₽', value: Math.round(potential * coefficient) }); await this.audit(req.user.userId, req.user.schoolId, 'REPORT_EXPORT', 'Period', periodId, { applications: apps.length, fund, coefficient }); res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); res.setHeader('Content-Disposition', `attachment; filename="report-${periodId}.xlsx"`); await wb.xlsx.write(res); res.end(); }

  private onlyDeputy(u: Session) { if (u.role !== Role.DEPUTY) throw new ForbiddenException({ code: 'DEPUTY_ONLY', message: 'Доступ только заместителю директора' }); }
  private fundAmountOf(raw: any): number | null {
    if (raw === undefined || raw === null || raw === '') return null;
    const value = Number(raw);
    if (!Number.isInteger(value) || value < 0 || value > 1_000_000_000) throw new BadRequestException({ code: 'FUND_INVALID', message: 'Фонд должен быть целым числом от 0 до 1 000 000 000 ₽' });
    return value;
  }
...
async function bootstrap() { const app = await NestFactory.create(AppModule); app.enableCors(); app.setGlobalPrefix(''); const port = Number(process.env.PORT || 3001); await ensureDatabase(); await app.listen(port); console.log(`API listening on http://localhost:${port}`); }
