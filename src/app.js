import { loadState, saveState, createApplication, submitApplication, decideApplication, getReviewableApplications, updateCriterionAmount, criterionUpdatePayload, getQualityBand, getQualityBands, getOlympiadReward, calculateOlympiadTotal, normalizeQualityPercentage, getApplicationValidationErrors, calculatePayouts, categoryBandScales, categoryLabel, TEACHER_CATEGORIES, DEFAULT_CATEGORY_BANDS, OLYMPIAD_LEVELS, DIPLOMA_TYPES, QUALITY_BANDS } from './domain.js';
...
function mapCriterion(c) {
  ...
  return { id: c.id, type, title: c.title, category: c.category, amount: Number(c.amount ?? c.maxAmount ?? 0), maxAmount: Number(c.maxAmount || 0), categoryBands, qualityBands: fallbackBands.length ? fallbackBands : QUALITY_BANDS, olympiadAmounts, usage: usageCount ? `${usageCount} ${usageCount === 1 ? 'заявка' : usageCount < 5 ? 'заявки' : 'заявок'}` : 'не использовался', allowEvidence: Boolean(c.allowEvidence), active: c.active !== false, version: c.version, fields, scales: c.scales || [] };
}
...
function bind() {
  ...
  document.querySelector('[data-criterion-form] select[name="type"]')?.addEventListener('change', (event) => { captureCriterionEditorForm(); criterionEditor.type = event.target.value; if (criterionEditor.type === 'quality' && !criterionEditor.categoryBands) criterionEditor.categoryBands = structuredClone(DEFAULT_CATEGORY_BANDS); render(); });
  ...
  document.querySelectorAll('[data-toggle-row]').forEach((el) => {
    ...
    el.addEventListener('keydown', (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); toggle(event); } });
  });
