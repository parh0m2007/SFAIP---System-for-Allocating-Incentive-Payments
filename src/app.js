...
function criterionEditorView() {
  ...
  const fields = c.fields?.length ? c.fields : [{ key: 'field_1', label: '', type: 'TEXT', required: false, options: [] }];
...
function bind() {
  ...
  document.querySelector('[data-action="new-criterion"]')?.addEventListener('click', () => { criterionEditor = { type: 'fixed', category: 'Другое', maxAmount: 0, amount: 0, allowEvidence: false }; render(); });
